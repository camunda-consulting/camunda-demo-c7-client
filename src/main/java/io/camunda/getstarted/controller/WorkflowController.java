...
public class WorkflowController {
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value = "/process/start", method = RequestMethod.POST, consumes = {"application/json"})
	public HttpResponse start(@RequestBody(required = true)String data)
		...
		workflowData.put("messageName", jsData.get("processKey"));

		JSONObject processVariables = new JSONObject();
		Iterator<String> itr = jsData.keys();
		workflowData.put("businessKey", jsData.getString("caseId"));
		while (itr.hasNext())
		{
			String key = itr.next();
			JSONObject value = new JSONObject().put("value", jsData.getString(key));
